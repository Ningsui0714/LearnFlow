...
**v5 是唯一正式契约**。v4 文件（`连续学习个性化讲解工作流_v4.yml`、`测验后个性化错误讲解工作流_v4.yml`）与 `history/` 目录中的 v2/v3 旧版均已删除，不再保留历史参考。
...
| `个性化推荐工作流.yml` | ✅ 正式 | 个性化推荐 |
| `目标与路径规划工作流.yml` | ✅ 正式 | 目标与路径规划 |
| `对话问答工作流.yml` | ✅ 正式 | 对话问答（新增，对应 `XINGCHEN_CHAT_FLOW_ID`） |
| `测评出题工作流.yml` | ✅ 正式 | 测评出题（新增，对应 `XINGCHEN_QUIZ_FLOW_ID`） |
