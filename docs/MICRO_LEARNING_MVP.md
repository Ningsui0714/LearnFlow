...
讲解、苏格拉底追问或费曼复述；项目是对话可挂载的长期上下文。只有学习者明确提出
...
产品事件分析至少按 `run_id` 统计：开始率、学习卡到复述转化、复述到验证转化、首题正确率、纠错进入/完成率、本轮完成率、暂停恢复率、完成用时、复习到期参与率。阶段二先把苏格拉底与费曼升级为对话内可恢复状态机，并复用本流程作为独立验证附件；详细契约见 `docs/CONVERSATION_SKILL_RUNTIME.md`。SQ3R、康奈尔、番茄或西蒙的完整运行时仍应由行为数据与定性访谈决定，而不是按功能数量决定。
