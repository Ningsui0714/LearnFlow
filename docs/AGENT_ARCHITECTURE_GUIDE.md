...
文件关联与变更事件的 kernel target 为空。编辑成功、保存草稿和练习“运行”都不是掌握证据；只有播放器内的正式练习提交可进入评估链。后续本地代码 Agent 只能通过 Tutor 所有的 Broker 工具在隔离副本中工作，不新增第四类主 Agent。安全细节以 `docs/DESKTOP_WORKSPACE_SECURITY.md` 为准。

## 9. 五核学习者模型

五核是状态维度，不是五个聊天 Agent。

| Kernel | 回答的问题 | 典型短期内容 | 典型长期内容 |
|---|---|---|---|
| structure | 学习者走到哪里，怎样离开与返回 | 当前关卡、依赖、转向、阻塞、返回锚点 | 稳定学习路径模式与项目图谱 |
| knowledge | 对哪个知识点理解到什么程度 | 知识缺口、待解问题、近期错误、明确误解 | 有独立证据支持的稳定掌握或误解模式 |
| human | 当前怎样教更合适 | 情绪、负荷、注意力、挫败、支持需要 | 多 session 一致或用户确认的稳定偏好 |
| value | 为什么学，什么更值得投入 | 当前优先级、动机、兴趣信号、目标候选 | 明确长期目标、职业方向与价值排序 |
| practice | 能否独立做出来 | 当前尝试、辅助等级、产物状态、近期反馈 | 独立实践能力与迁移证据 |
