...
const plugin = defineLearnFlowPlugin({
  manifest: {
    ...
    tools: [
      ...
      {
        id: 'read_capability_radar', title: '读取能力雷达', description: '从岗位包正式能力视图读取顶层能力轴和证据强度投影；该数值不是学习者分数。',
        whenToUse: '用户询问岗位核心能力、能力结构、优势维度，或明确希望查看能力雷达时。',
        whenNotToUse: '不要用它评价学习者能力，不要把证据强度当作市场占比或统计概率。',
    ...
    skills: [{
      ...
      instructions: [
        ...
        '只有局部问题没有稳定对象 ID 时才调用 search_role_knowledge；已有 ID 时精确读取；能力结构用 read_capability_radar；解释关系时查询图；解释工作如何发生时追踪事理过程。',
    ...
    renderers: [
      ...
      { id: ROLE_RENDERERS.radar, title: '能力雷达', description: '显示顶层岗位能力轴及其证据强度投影。' },
