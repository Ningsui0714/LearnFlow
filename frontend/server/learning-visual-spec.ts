...
function visualPlannerPrompt(kind: LearningVisualKind, domain: LearningVisualDomain, abstraction: LearningVisualAbstraction) {
  const timeline = kind === 'diagram'
    ...
    : '动画必须给出 initialState、1-12 个 frames、invariants、finalState。每帧必须有 patches 且重放后真正改变状态；禁止只给 activeNodeIds/activeRelationIds。patch type 仅允许 send_message、transition_state、move_item、set_pointer、set_active_line、set_variable、push_stack、pop_stack、set_tensor_shape、set_parameter、set_probability_sample、replace_series、transform_object、replace_expression；patch 必须匹配当前 semantic.type，不能跨抽象借用。'
  return `你是 LearnFlow 教学视觉语义规划器。只输出一个 JSON 对象，不输出 SVG、HTML、Mermaid、脚本、代码围栏或可执行表达式。\n\n目标：${kind}\n领域：${domain}\n抽象：${abstraction}\nsemantic 必须严格使用：${semanticSchemaHint(domain, abstraction)}\n\n共同字段：{"version":"learnflow.visual.v2","kind":"${kind}","title":"短标题","subtitle":"阅读提示","domain":"${domain}","abstraction":"${abstraction}","semantic":{...},"accessibility":{"summary":"完整文字摘要","readingOrder":["有效对象id"],"nonColorStateCue":"非颜色状态提示"},"explanation":"简短教学说明"}。${timeline}\n\n所有 ID 必须是小写 ASCII 稳定 ID；所有引用必须存在；关系缺失时不得猜测或自动连线。函数、分布和变换只能提供有限数值采样点，不能提供待 eval 的表达式。代码 trace 只是转义后的展示数据，绝不要求执行模型代码。无法确认的数值、关系或中间状态不要编造。`
}
...
export async function generateLearningVisual(
  ...
  try {
    ...
    if (payload.nodes !== undefined || (Array.isArray(payload.frames) && payload.semantic === undefined)) {
      ...
      modelError = kind === 'animation' ? 'animation_requires_typed_semantic_patches' : 'legacy_visual_plan_not_v2'
