...
function PluginReleaseCard({
  ...
  return <article className="project-plugin-card">
    ...
    <footer>
      ...
      {instance?.status === 'enabled' && <button type="button" className="project-plugin-open-workspace" disabled={busy} onClick={() => onOpenPluginWorkspace(group.pluginId)}>{group.pluginId === 'learning_task_conversion' ? '打开任务工作台' : '在 Tutor 对话中使用'}</button>}
