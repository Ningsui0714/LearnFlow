...
function submittedIssueCount(bundle: LearningTaskConversionBundle) {
  ...
  return items.reduce((total: number, item: any) => (
    total + (Array.isArray(item?.issues) ? item.issues.length : 0)
  ), 0)
}
    ...
    <div className="flex h-full min-h-0 bg-slate-100">
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-12 shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4">
          ...
          <a href={bundle.artifacts.personalized_learning_json_url} target="_blank" rel="noreferrer" className="flex h-8 items-center gap-1.5 border border-slate-200 px-2.5 text-[10px] text-slate-600 hover:bg-slate-50">
        ...
        >
          <WF03TaskDocument bundle={bundle} />
        </div>
      </section>
