import {
  ArrowDown, BookOpenCheck, CheckCircle2, ClipboardCheck, Database, Wrench,
} from 'lucide-react'
...
export default function WF03TaskDocument({ bundle }: { bundle: LearningTaskConversionBundle }) {
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {step.knowledge_point_ids.map((id, mappingIndex) => (
                        <span key={`${id}-${mappingIndex}`} className="border border-indigo-200 bg-indigo-50 px-2 py-1 text-[10px] text-indigo-700">
                          知识 · {knowledgeById.get(id)?.name || id}
                        </span>
                      ))}
