...
任务只保存目标；状态由浏览器本地追加式事件队列重建。任务不拥有统一的教学阶段，当前
...
Skill 切换、完整步骤、循环、暂停、结束和事件记录收进选项菜单，不建立竞争性的任务详情页。
...
事件只追加、带 Conversation 内递增序号。当前原型记录创建、开始、Skill 步骤进入、Skill
循环、学生回应、支架请求、Skill 切换、暂停、恢复和流程完成。页面刷新后从 `localStorage`
恢复并重新投影。
