...
export const LEARNING_SKILLS: Record<LearningSkillId, LearningSkillDefinition> = {
  guided_explanation: {
    ...
    steps: [
      {
        id: 'anchor_model', title: '建立最小模型', shortTitle: '模型', nextAction: '看最小例子', canLoop: true,
      ...
      {
        id: 'inspect_example', title: '检查最小例子', shortTitle: '例子', nextAction: '让我解释', canLoop: true,
        tutorInstruction: '给一个足够小、能逐项映射到核心模型的例子，明确例子中每个部分对应什么，然后只让学生判断一个关键步骤。',
        loopInstruction: '保留同一知识关系，换一个更具体或更小的例子，并把待判断范围缩小。',
      },
      {
        id: 'learner_explain', title: '学生解释关键关系', shortTitle: '复述', nextAction: '换情境检查', canLoop: true, requiresLearnerReply: true,
      ...
      {
        id: 'transfer_check', title: '换情境检查', shortTitle: '迁移', nextAction: '完成本轮', canLoop: true, requiresLearnerReply: true,
  ...
  socratic_dialogue: {
    ...
    steps: [
      {
        id: 'ground_context', title: '提供可回答起点', shortTitle: '起点', nextAction: '提出判断', canLoop: true, requiresLearnerReply: true,
      ...
      {
        id: 'hypothesis', title: '提出一个判断', shortTitle: '假设', nextAction: '追问理由', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '邀请学生对具体情境提出一个可检验判断；一次只问一个问题，不提前给完整答案。',
        loopInstruction: '缩小判断范围，给出两个候选方向或固定一个变量，让学生只选择并解释一项。',
      },
      {
        id: 'probe_reason', title: '追问判断理由', shortTitle: '理由', nextAction: '检验边界', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '围绕学生刚才的判断只追问一个“为什么成立”的关键连接；先回应已有推理，不要机械重复问题。',
        loopInstruction: '把理由拆成前提到结论之间缺失的一步，提供句子开头或局部事实后再问。',
      },
      {
        id: 'test_boundary', title: '用边界检验假设', shortTitle: '边界', nextAction: '收束推理', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '给一个反例、极端值或改变单一条件的情境，请学生判断原假设是否仍成立以及为什么。',
        loopInstruction: '降低反例复杂度，只改变一个条件，并明确其余条件保持不变。',
      },
      {
        id: 'synthesize_reasoning', title: '学生收束推理', shortTitle: '收束', nextAction: '完成本轮', canLoop: true, requiresLearnerReply: true,
  ...
  feynman_dialogue: {
    ...
    steps: [
      {
        id: 'knowledge_anchor', title: '补齐必要起点', shortTitle: '起点', nextAction: '开始复述', canLoop: true,
        tutorInstruction: '用很短的说明确认学生已经拥有复述所需的对象和关系；若尚未接触主题，先给最小讲解，不能直接要求复述未知内容。',
        loopInstruction: '换成一个更直观的最小模型或例子，只补足复述必需的知识。',
      },
      {
        id: 'first_teachback', title: '第一次用自己的话讲', shortTitle: '初讲', nextAction: '定位跳步', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '指定听众和范围，请学生不用术语堆砌地讲清一个机制。收到回答后先指出讲清的一点，不把流畅复述当作掌握。',
        loopInstruction: '把复述目标缩小到一个关系，并给出“它先……所以……”的句架。',
      },
      {
        id: 'diagnose_gap', title: '定位一个关键跳步', shortTitle: '诊断', nextAction: '修订复述', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '只指出复述中最关键的一个模糊处、遗漏前提或术语替代解释，并问一个能暴露该连接的问题。',
        loopInstruction: '把跳步拆成更小的前提问题；如仍不会，直接补足该前提，再保留修订机会。',
      },
      {
        id: 'revised_teachback', title: '带着修正再讲一遍', shortTitle: '修订', nextAction: '补例子与边界', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '请学生只修订刚才的关键跳步，再把它放回完整解释。对比前后变化，但不做掌握判断。',
        loopInstruction: '继续围绕同一跳步缩小复述范围；必要时给半成品解释让学生改错。',
      },
      {
        id: 'example_or_boundary', title: '补一个例子或边界', shortTitle: '边界', nextAction: '完成本轮', canLoop: true, requiresLearnerReply: true,
  ...
  worked_example_fading: {
    ...
    steps: [
      {
        id: 'worked_example', title: '观看带子目标的示范', shortTitle: '示范', nextAction: '补最后一步', canLoop: true,
        tutorInstruction: '给一个小而完整、按子目标标注的示例，解释每一步为什么服务于目标；不要一次塞入多个变体。',
        loopInstruction: '换一个更小的输入或补充逐行标注，仍展示完整过程。',
      },
      {
        id: 'complete_last_step', title: '补全最后一步', shortTitle: '末步', nextAction: '补中间步骤', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '保留前面过程，只撤掉最后一个可检查步骤，让学生补全并说明该步骤如何得到。',
        loopInstruction: '给最后一步的输入、输出形状或一个局部提示，仍由学生完成该步。',
      },
      {
        id: 'complete_middle_step', title: '补全中间步骤', shortTitle: '渐隐', nextAction: '独立做变式', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '撤掉一个中间步骤及其后续结果，让学生补全当前步骤；每轮只处理一个缺口。',
        loopInstruction: '恢复一个相邻步骤或给出待用规则，降低一次需要保持的信息量。',
      },
      {
        id: 'independent_problem', title: '独立完成近迁移变式', shortTitle: '变式', nextAction: '解释策略', canLoop: true, requiresLearnerReply: true,
        tutorInstruction: '给一个表面不同但使用同一策略的小变式，先不提供步骤。若学生请求提示，明确记录支架并留在本步，不能视为独立完成。',
        loopInstruction: '再给一个难度相近的变式；若上一轮用了提示，先换题再尝试无提示完成。',
      },
      {
        id: 'reflect_strategy', title: '说出策略选择依据', shortTitle: '策略', nextAction: '完成本轮', canLoop: true, requiresLearnerReply: true,
...
export function createLearningTask(objective: string, now = Date.now(), existingEvents: LearningEvent[] = []) {
  ...
  const events = appendLearningEvents(existingEvents, task.id, [
    ...
    { type: 'vnext_learning_skill_step_entered', detail: `进入${firstStep.title}`, skillId, stepId: firstStep.id },
  ], now)
  return { task, events }
}
...
export function advanceLearningSkillStep(events: LearningEvent[], projection: LearningTaskProjection, now = Date.now()) {
  ...
  return appendLearningEvents(events, projection.task.id, [{
    ...
    detail: `进入${next.title}；不表示上一动作通过`,
...
export function switchLearningSkill(events: LearningEvent[], projection: LearningTaskProjection, skillId: LearningSkillId, now = Date.now()) {
  ...
  return appendLearningEvents(events, projection.task.id, [
    ...
    { type: 'vnext_learning_skill_step_entered', detail: `从${firstStep.title}开始`, skillId, stepId: firstStep.id },
  ], now)
}
