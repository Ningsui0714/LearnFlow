...
const vnextDir = path.resolve(scriptDir, '..')
const rootDir = path.resolve(vnextDir, '..')
const backendDir = path.join(rootDir, 'backend')
const pythonBin = path.join(backendDir, 'venv', 'bin', 'python')
const viteBin = path.join(vnextDir, 'node_modules', 'vite', 'bin', 'vite.js')

async function readLocalEnv() {
  try {
    const source = await readFile(path.join(vnextDir, '.env.local'), 'utf8')
...
try {
  if (localBackend && !await backendIsHealthy()) {
    ...
    console.log(`[LearnFlow vNext] 启动正式后端：${backendBase}`)
    ...
    backendProcess.once('error', error => {
      console.error(`[LearnFlow vNext] 正式后端启动失败：${error.message}`)
      shutdown(1)
    })
    await waitForBackend()
    backendProcess.once('exit', code => {
      if (!shuttingDown) {
        console.error(`[LearnFlow vNext] 正式后端已退出（exit ${code ?? 'unknown'}）`)
        shutdown(code || 1)
      }
    })
  } else if (localBackend) {
    console.log(`[LearnFlow vNext] 复用已运行的正式后端：${backendBase}`)
  } else {
    console.log(`[LearnFlow vNext] 使用外部正式后端：${backendBase}`)
  }

  console.log('[LearnFlow vNext] 启动页面：http://127.0.0.1:4174')
  viteProcess = spawn(
    process.execPath,
    [viteBin, '--host', '127.0.0.1', '--port', '4174', '--strictPort'],
    {
      cwd: vnextDir,
      env: { ...process.env, VNEXT_BACKEND_URL: backendBase },
      stdio: 'inherit',
    },
  )
  viteProcess.once('error', error => {
    console.error(`[LearnFlow vNext] 页面服务启动失败：${error.message}`)
    shutdown(1)
  })
  viteProcess.once('exit', code => shutdown(code ?? 1))
} catch (error) {
  console.error(`[LearnFlow vNext] ${error instanceof Error ? error.message : String(error)}`)
  shutdown(1)
}
